...
if(isset($_GET['nis'])){
    ...
    while($id_pelanggaran_siswa = mysqli_fetch_assoc($pelanggaran_siswa)){
        ...
        if(mysqli_num_rows($cek_data) == 0){
            echo "<script>alert('Siswa melakukan pelanggaran baru, silahkan cetak surat lagi, dengan memilih nis " . $nis . "'); window.location.href = '/Poin_Pelanggaran_Siswa_XIIRPL3/pages/cetak/add_perjanjian_ortu.php';</script>";
            exit;
        }
    }
    ...
    if(mysqli_num_rows(mysqli_query($conn, "SELECT * FROM pelanggaran_siswa WHERE nis = '$nis'")) == 0){
        echo "<script>alert('Data siswa belum memiliki data pelanggaran 👍'); window.location.href = '/Poin_Pelanggaran_Siswa_XIIRPL3/pages/cetak/add_perjanjian_ortu.php';</script>";
        exit;
    }
